...
基于 `yipack` 的项目，目录结构都有其特定含义，请同学们仔细阅读以下说明，以便更加熟练地使用 `yipack` 开发项目。
...
`yipack` 采取的是【约定大于配置】的原则，`pages` 便是这个原则指导下，一个非常重要的目录。
...
使用 `yipack new --page 页面名称`，可以自动在 `pages` 目录下，生成对应的页面目录和文件
...
与其他框架不同的是，`yipack` 不仅提供一键创建全局组件命令`yipack new --comp 组件名称`。
...
路由，是 vue 单页应用的灵魂之一，为了简便路由的管理，`yipack` 路由目录下，只有一个 `index.js` 文件。
...
综合实践经验，`yipack` 抛弃传统的单接口服务器方式，默认采用了多接口服务器配置。
...
为了提高开发效率，`yipack` 全局存储默认提供了 `save` 设置数据的通用方法。
